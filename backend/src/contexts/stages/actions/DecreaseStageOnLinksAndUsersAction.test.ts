...
import { DecreaseStageOnLinksAndUsersAction } from './DecreaseStageOnLinksAndUsersAction';

describe('Decrease stage on links and users action', () => {
  ...
  // test('decrease stage on one link and one user when there are not votes', async () => {
  //   jest.setSystemTime(new Date(2023, 3, 7));
  //   await expect(decreaseStageOnLinksAndUsersAction.execute()).rejects.toThrow(
  //     NoVotesOnThisCycleException
  //   );
  // });

  test('decrease stage in one link and one user', async () => {
    const stageInitial = 2;
    ...
    await linkRepository.storeLink(link);
    ...
    jest.setSystemTime(new Date(2023, 3, 7));
    await decreaseStageOnLinksAndUsersAction.execute();
